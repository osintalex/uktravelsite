...
def convert_country_names():
    ...
    """
    dataframes = {}
    outbound_file_path = os.getcwd() + "/corona/data/outbound-graph-data.csv"
    inbound_file_path = os.getcwd() + "/corona/data/inbound-graph-data.csv"

    for csv_path in [outbound_file_path, inbound_file_path]:
        ...
        if csv_path == outbound_file_path:
            dataframes['outbound'] = df
        elif csv_path == inbound_file_path:
...
def make_outbound_graph(df):
    ...
    df["Coronavirus Information"] = df["Coronavirus Information"].apply(lambda x: "<br>".join(textwrap.wrap(x)))
    df["Quarantine Information"] = df["Quarantine Information"].apply(lambda x: "<br>".join(textwrap.wrap(x)))
